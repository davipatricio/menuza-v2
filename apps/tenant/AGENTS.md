...
- Compiled artifact: `bun run build` → `dist/tenant` via `bun build --compile --bytecode --format=esm --minify --sourcemap` (no tsdown). Images build via `bun run scripts/docker-build.ts tenant`.
