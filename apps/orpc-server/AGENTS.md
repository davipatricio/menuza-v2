...
- Router implementations live in the consuming apps (`apps/commerce`, `apps/tenant`) under
  `src/domains/<domain>/subdomains/<subdomain>/contracts/` (Zod + oRPC contract) and
  `<subdomain>/<name>.impl.ts` (procedure implementation). Domain routers aggregate subdomains.
- Exports `initSentry({ service })` and `getTracer()` for observability. Sentry is a no-op
  without `SENTRY_DSN`; OpenTelemetry has no exporter until one is added by the consumer.
