...
- Deny-by-default: the health probe, the push routes, the staff session, panel and
  profile routes (`session.*`, `panel.getStore` / `panel.createStore`,
  `profile.saveOnboarding`), and the internal `internal.resolveHost` procedure are
  exposed; everything else is absent. `resolveHost` requires the shared internal
  token; the session/panel/profile routes resolve the member from the
